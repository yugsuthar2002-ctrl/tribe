...
const ProjectDetail: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-brand-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/portfolio" className="text-brand-purple hover:text-white transition-colors">
            ← Back to Portfolio
          </Link>
        </div>
      </div>
  ...
  return (
    <section className="min-h-screen bg-brand-black text-white py-32">
      <div className="max-w-7xl mx-auto px-6">
        <FadeIn>
          <Link to="/portfolio" className="text-brand-purple hover:text-white transition-colors mb-8 inline-block">
            ← Back to Portfolio
          </Link>
        </FadeIn>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
          <FadeIn delay={0.4}>
            <div className="space-y-8">
              <div>
                <h2 className="text-2xl font-bold mb-4 text-brand-purple">Project Overview</h2>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((tech) => (
                    <span key={tech} className="px-3 py-1 bg-brand-purple/20 text-brand-purple rounded-full text-sm font-medium">
                      {tech}
                    </span>
                  ))}
                </div>
            ...
            <div className="glass-card p-8 rounded-2xl">
              ...
              <div className="grid grid-cols-3 gap-4">
                {project.images.map((image, index) => (
                  <div key={index} className="aspect-square bg-gradient-to-br from-brand-purple/20 to-brand-cyan/20 rounded-xl flex items-center justify-center text-4xl">
                    {image}
                  </div>
                ))}
              </div>
